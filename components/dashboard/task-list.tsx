...
export function TaskList() {
  ...
  const TaskItem = ({ task, showCategory = false }: { task: any; showCategory?: boolean }) => {
    ...
    return (
      <div className="group flex items-start space-x-3 p-4 bg-white/60 backdrop-blur-sm rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-md transition-all duration-200">
        <Checkbox
    ...
    <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-200">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          <DialogTrigger asChild>
            <Button
              size="sm"
              className="rounded-lg bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700 text-white shadow-md hover:shadow-lg transition-all duration-200"
            >
      ...
      <CardContent>
        <Tabs defaultValue="today" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-gray-100/80 backdrop-blur-sm rounded-lg p-1">
            <TabsTrigger value="today" className="rounded-md text-sm font-medium">
              Today ({todayTasks.length})
            </TabsTrigger>
            <TabsTrigger value="upcoming" className="rounded-md text-sm font-medium">
          ...
          <TabsContent value="upcoming" className="mt-4 space-y-3">
            {upcomingTasks.length === 0 ? (
              <div className="text-center py-8 bg-gradient-to-br from-blue-50 to-violet-50 rounded-xl border border-blue-100">
          ...
          <TabsContent value="completed" className="mt-4 space-y-3">
            {completedTasks.length === 0 ? (
              <div className="text-center py-8 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-100">
